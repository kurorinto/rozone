...
const RozoneLayer = () => {
  ...
  const requestCompletedHandler = useCallback((
    ...
    // 拦截规则匹配
    const isMatched = rules.some(({ value }) => value.mode === '1' ? details.url.includes(value.decs) : value.mode === '2' ? new RegExp(value.decs).test(details.url) : false)
  ...
  useEffect(() => {
    chrome.webRequest.onCompleted.addListener(requestCompletedHandler, {
      // todo: 拦截网址配置
      urls: ["<all_urls>"]
    })
    return () => {
